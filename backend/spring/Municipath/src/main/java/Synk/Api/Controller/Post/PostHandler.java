...
public class PostHandler {
    ...
    public boolean createPost(String title, PostType type, String text, String author, Position pos,
    	...
    	if(level < 2)
    		return false;
    	PostCreator creator = type == PostType.EVENT ? this.eventCreator : type == PostType.CONTEST ?
    	...
    	creator.setData(title, text, data);
    	creator.setDetails(author, pos, level > 3, type);
    	creator.setSpecialDetails(level > 2, start, end, persistence);
    	if(!creator.correctPost())
    		return false;
    	Post post = creator.createPost();
